...
class CollisionChecker:
	...
	def check_distinguished_points(self, distinguished_points, n):
		for i in range(n):
			# ints = np.frombuffer(self.distinguished_points[i]["inbuffer"]["buffer"].tobytes(), dtype=np.int32)
	...
	def verify(self, dp_0, dp_1, bits):
		...
		print(f"self.verify(\"{s_0}\", {o_0}, \"{s_1}\", {o_1}, {bits})")
		# print(dp_0["outbuffer"]["buffer"].tobytes().hex())
		# print(dp_1["outbuffer"]["buffer"].tobytes().hex())
		# self.verifier.verify(s_0, o_0, s_1, o_1, bits)
		dp_np = np.array([dp_0, dp_1])
		mf = cl.mem_flags
		dp_g = cl.Buffer(self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=dp_np)
